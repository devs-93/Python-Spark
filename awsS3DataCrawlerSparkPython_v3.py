...
import pandas as pd
from pyspark.sql.functions import first, current_timestamp
...
explode_df = multiline_normal_df.select('params', 'date').withColumn("params",
...
# explode_df = explode_df.withColumn('Record_Index', sf.lit('N'))
# print(type(explode_df))
explode_df.show()
exit()
explode_updated_df = explode_df.withColumnRenamed("current_timestamp()", "timeToGen")
explode_updated_df.printSchema()
new_df_data = explode_updated_df.groupBy("timeToGen", "date").pivot("key").agg(
    first('key').alias('key_data'),
    first('string_value').alias('string_value'),
    first("int_value").alias("int_value"),
    first('float_value').alias('float_value'),
    first("double_value").alias("double_value"))

shukla = new_df_data.toPandas()
shukla.to_csv(DESTINATION_LOCATION)
# data = explode_df.show(vertical=True)
...
df_row_merged.to_csv(DESTINATION_LOCATION, index=False, header=True)
